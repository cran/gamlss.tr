...
fun <- if (type=="left")  
      ...
      cof
       ...
       function(q, lower.tail = TRUE, log.p = FALSE, ...)
      {
      if (any(q > par))  stop(paste("q must be less or equal to ", par, "\n", ""))
      cof <- cdf(q,...)/cdf(par,...)
      cof <- if(lower.tail == TRUE) cof  else 1-cof   
      cof <- if(log.p==FALSE)  cof else  log(cof) 
      cof
      ...
      if (distype=="Discrete" &&  (any(q <= par[1]) || any(q > par[2])) )  
         stop(paste("q must be greater than", par[1], "and less than", par[2]+1, 
                      "\n", ""))
      if (distype!="Discrete" && (any(q < par[1]) || any(q > par[2])) )
        stop(paste("q must be greater than", par[1], "and less or equal to", par[2], 
                      "\n", ""))  
      cof <- (cdf(q,...)-cdf(par[1],...))/(cdf(par[2],...)-cdf(par[1],...))
      cof <- if(lower.tail == TRUE) cof  else 1-cof   
      cof <- if(log.p==FALSE)  cof else  log(cof) 
      cof
      }
